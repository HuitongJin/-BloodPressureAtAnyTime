...
import com.example.seismo.mysql.DBConnectHelper;
...
public class PPG extends AppCompatActivity{
    ...
    private int addX = -1;
    private static HashMap<Long, Integer> all = new HashMap<>();
    double addY;
    int [] xv = new int[300];
    int [] yv = new int[300];
    ...
    private void updateChart() {
        ...
        if (flag == 1) {
            ...
            if (gx < 200) {
                if (hua[20] > 1) {
                    Toast.makeText(PPG.this, "请用您的指尖盖住摄像头镜头！", Toast.LENGTH_SHORT).show();
                    hua[20] = 0;
                }
                hua[20]++;
                return;
    ...
    private static Camera.PreviewCallback previewCallback = new Camera.PreviewCallback() {
        ...
        public void onPreviewFrame(byte[] data, Camera camera) {
            ...
            int imgAvg = (int)ImageProcessing.decodeYUV420SPtoRedBlueGreenAvg(data.clone(), height, width, 1);
            gx = imgAvg;
            all.put(imTime, imgAvg);
